...
public:
    ...
    summarizer::SummarizeConfig makeConfig(std::filesystem::path const & video_path);
...
public:
    void InsertFile(std::filesystem::path const & file_name);
    void InsertFileList(std::vector<std::filesystem::path> const & file_list);

private:
    void InsertFileToQueue(std::filesystem::path const & file_name);
    void InsertFileListToQueue(std::vector<std::filesystem::path> const & file_list);
    std::wstring PopQueue();
...
private:
    ...
    std::queue<std::filesystem::path> target_queue_;
